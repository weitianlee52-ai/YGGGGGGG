 ...
 \ \   __  \\ \   _  _\\ \  \\\  \\ \  \_|/__\ \  \     \ \  \\\  \\ \  \  
  \ \  \ \  \\ \  \\  \|\ \  \\\  \\ \  \_|\ \\ \  \____ \ \  \\\  \\ \  \ 
   \ \__\ \__\\ \__\\ _\ \ \_____  \\ \_______\\ \_______\\ \_______\\ \__\
    ...
    ----- LUARMOR 模式: 卡密系统已对接 Luarmor -----
]]

repeat task.wait() until game:IsLoaded()

-- LUARMOR 模式: 加载 Luarmor SDK 并设置脚本 ID
local LuarmorAPI = loadstring(game:HttpGet("https://sdkapi-public.luarmor.net/library.lua"))()
LuarmorAPI.script_id = "7fad47e103bed632a2670fc3972e53c3"  -- 请替换为你的脚本 ID
...
-- 主题
Arqel.Theme = {
    Accent = Color3.fromRGB(255, 105, 180),        -- 粉色主色
    AccentHover = Color3.fromRGB(255, 135, 200),   -- 亮粉色悬停
    ...
    Error = Color3.fromRGB(255, 80, 120),          -- 粉红错误色
    Warning = Color3.fromRGB(255, 255, 0),
    StatusIdle = Color3.fromRGB(220, 80, 130),     -- 粉色状态色
    Discord = Color3.fromRGB(255, 105, 180),       -- Discord按钮粉色
    DiscordHover = Color3.fromRGB(255, 135, 200),  -- Discord悬停粉色
...
-- LUARMOR 模式: 定义 Luarmor 验证函数
function Arqel:LuarmorVerify(key)
    if not key or key == "" then
        return { valid = false, error = "KEY_EMPTY", message = "请输入卡密" }
    end
    local status = LuarmorAPI.check_key(key)
    if status.code == "KEY_VALID" then
        return { valid = true }
    else
        -- 将 Luarmor 的状态码映射为用户友好的提示
        local friendlyMsg = status.message
        if status.code == "KEY_HWID_LOCKED" then
            friendlyMsg = "卡密已绑定到其他硬件ID，请重置。"
        elseif status.code == "KEY_INCORRECT" then
            friendlyMsg = "卡密错误或已被删除！"
        elseif status.code == "KEY_EXPIRED" then
            friendlyMsg = "卡密已过期。"
        elseif status.code == "KEY_BLACKLISTED" then
            friendlyMsg = "此卡密已被列入黑名单。"
        end
        return { valid = false, error = status.code, message = friendlyMsg }
    end
end
...
local function getDeviceType()
    ...
    if gamepad and not keyboard and not touch then return "游戏主机"
    elseif touch and not keyboard then return "手机/平板"
    elseif keyboard and touch then return "PC + 触屏"
...
local function ShowLoadingScreen(onComplete)
    ...
    local phases = {}
    local phaseNames = {"初始化", "创建文件夹", "下载资源", "准备界面", "就绪"}
...
local function CreateUserInfoPanel(parent, windowWidth, panelHeight, panelWidth, mainFrame, gap, startOpen)
    ...
    copyBtn.MouseButton1Click:Connect(function()
        ...
        Arqel:Notify("已复制", "硬件ID已复制到剪贴板", 2, "copy")
    end)
...
local function handleKeylessSkip()
    ...
    getgenv().ArqelLoaded = false
    Arqel:Notify("访问已授予", "无卡密模式已授权！", 3, "success")
    task.wait(0.3)
...
local function BuildKeylessUI()
    ...
    launchBtn.MouseButton1Click:Connect(function()
        Arqel:Notify("启动中", "脚本加载成功！", 2, "success")
        getgenv().SCRIPT_KEY = "KEYLESS"
        getgenv().ArqelLoaded = false
        closeDoorsThenExit(function()
            disableBlur()
...
local function BuildKeyUI()
    ...
    textBox.PlaceholderText = "请输入您的卡密..."
    ...
    local acquireBtn = createButton("获取卡密", "user", false, acquireStartY)
    ...
    if showShop then
        ...
        buyBtn.MouseButton1Click:Connect(function()
            if Arqel.Shop.Link ~= "" then
                ...
                Arqel:Notify("商店", "商店链接已复制到剪贴板！", 2, "copy")
            end
        end)
    end
    ...
    local function setStatus(state, customText)
        ...
        if state == "verifying" then
            ...
            dotsThread = task.spawn(function()
                while statusLabel and statusLabel.Parent and statusLabel.Text:find("正在验证卡密", 1, true) do
                    statusLabel.Text = text .. dots[i] i = (i % #dots) + 1 task.wait(0.4)
                end
            end)
        elseif state == "success" then color, icon, text = Arqel.Theme.Success, getIcon("check"), customText or "访问已授予"
        elseif state == "error" then color, icon, text = Arqel.Theme.Error, getIcon("alert"), customText or "无效卡密" end
    ...
    local function handleRedeem()
        ...
        if key == "" then Arqel:Notify("错误", "请输入您的卡密", 3, "warning") return end
        setStatus("verifying") redeemBtn.Active = false task.wait(0.3)
        local valid, errorMsg = false, "无效卡密"
        if Internal.ValidateFunction then
            ...
            if success then
                if type(result) == "table" then
                    ...
                    local errMsgs = {
                        KEY_INVALID = "系统中未找到卡密", KEY_EXPIRED = "卡密已过期",
                        HWID_BANNED = "硬件已被封禁", KEY_INVALIDATED = "卡密已被撤销",
                        ALREADY_USED = "一次性卡密已使用", HWID_MISMATCH = "硬件ID已达上限",
                ...
                elseif type(result) == "boolean" then valid = result errorMsg = msg or "无效卡密" end
            end
        end
        redeemBtn.Active = true
        if valid then
            saveKey(key) getgenv().SCRIPT_KEY = key getgenv().ArqelLoaded = false
            setStatus("success") Arqel:Notify("成功", "卡密验证通过！", 2, "success") task.wait(1)
            closeDoorsThenExit(function()
                disableBlur()
                TweenService:Create(container, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {Position = UDim2.new(0.5, 0, -0.5, 0)}):Play()
                TweenService:Create(mainFrame, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
    ...
    acquireBtn.MouseButton1Click:Connect(function()
        if Arqel.Links.GetKey ~= "" then Arqel:Notify("已复制", "卡密获取链接已复制！", 3, "copy") pcall(function() setclipboard(Arqel.Links.GetKey) end)
        else Arqel:Notify("错误", "未设置卡密链接", 3, "warning") end
    end)
    discordBtn.MouseButton1Click:Connect(function()
...
function Arqel:Launch()
    -- LUARMOR 模式: 强制使用 Luarmor 验证，覆盖用户自定义 OnVerify
    Arqel.Callbacks.OnVerify = function(key) return Arqel:LuarmorVerify(key) end
    ...
    if existingKey and existingKey ~= "" then
        if existingKey == "KEYLESS" then
            Arqel:Notify("已执行", "脚本加载成功！", 2, "success")
    ...
    EnsureIconsReady(function()
        ...
        if Arqel.Storage.AutoLoad and Internal.ValidateFunction then
            ...
            if savedKey and savedKey ~= "" then
                Arqel:Notify("检查中", "正在验证保存的卡密...", 2, "shield") task.wait(0.5)
                if validateKey(savedKey, Internal.ValidateFunction) then
                    getgenv().SCRIPT_KEY = savedKey
                    Arqel:Notify("欢迎回来", "卡密有效！", 2, "success")
                    if Arqel.Callbacks.OnSuccess then Arqel.Callbacks.OnSuccess() end return
...
function Arqel:LaunchJunkie(config)
    -- 为保持完整性保留此函数，但实际验证由 Luarmor 处理。若有 Junkie 需求可自行修改。
    assert(config and config.Service and config.Identifier and config.Provider, "配置缺失: Service, Identifier, Provider")
    ...
    EnsureIconsReady(function()
        ...
        if not success or not Junkie then Arqel:Notify("错误", "加载 Junkie SDK 失败", 5, "error") return end
